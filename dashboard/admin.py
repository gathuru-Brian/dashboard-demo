...
Super-admin workspace for:
  - Notifications: reviewing and actioning access requests
  ...
  - Reviewing uploaded datasets and applying retention policy
...
def _notifications_section() -> None:
    ...
    pending = get_access_requests("pending")

    if not pending:
        st.success("No pending access requests. You're all caught up.")
    else:
        st.caption(f"{len(pending)} pending access request(s) awaiting review.")

    for req in pending:
        with st.container(border=True):
            header_cols = st.columns([3, 1])
            header_cols[0].markdown(f"**{req['fullname']}**")
            header_cols[0].caption(f"{req['email']} • {req['department'] or 'No department given'} • {req['created_at']}")
            header_cols[1].markdown(
                "<span style='background:#F59E0B22;color:#F59E0B;padding:2px 10px;"
                "border-radius:10px;font-size:12px;'>Pending</span>",
                unsafe_allow_html=True,
            )
            if req["message"]:
                st.caption(f"\u201c{req['message']}\u201d")

            approve_key = f"approve_toggle_{req['id']}"
            action_cols = st.columns([1, 1, 2])
            if action_cols[0].button("Approve", key=f"approve_{req['id']}", icon=":material/check:", type="primary"):
                st.session_state[approve_key] = True
            if action_cols[1].button("Dismiss", key=f"dismiss_{req['id']}", icon=":material/close:"):
                update_access_request_status(req["id"], "rejected", reviewed_by=st.session_state.get("user", ""))
                st.rerun()

            if st.session_state.get(approve_key):
                st.markdown("**Create account for this request**")
                with st.form(f"approve_form_{req['id']}"):
                    col1, col2 = st.columns(2)
                    with col1:
                        role = st.selectbox("Role", get_roles(), key=f"role_{req['id']}")
                        department = st.selectbox(
                            "Department", get_departments(),
                            index=get_departments().index(req["department"]) if req["department"] in get_departments() else 0,
                            key=f"dept_{req['id']}",
                        )
                    with col2:
                        password = st.text_input(
                        ...
                        confirm = st.text_input("Confirm password", type="password", key=f"pwd2_{req['id']}")
                    confirm_create = st.form_submit_button("Create account and approve", type="primary", icon=":material/person_add:")
                ...
                if confirm_create:
                    ...
                    if not password_ok:
                        ...
                        st.error("The passwords do not match.")
                    elif user_exists(req["email"]):
                        st.error("An account already exists for this email address.")
                    else:
                        created = create_user(req["fullname"], req["email"], password, role, department)
                        if created:
                            update_access_request_status(req["id"], "approved", reviewed_by=st.session_state.get("user", ""))
                            notify_user_registration(req["email"], req["fullname"])
                            st.success(f"Account created for {req['fullname']}.")
                            del st.session_state[approve_key]
                            st.rerun()
                        else:
                            st.error("Could not create the account.")

    with st.expander("Recently reviewed requests"):
        reviewed = [r for r in get_access_requests(None, limit=100) if r["status"] != "pending"]
        if not reviewed:
            st.caption("No reviewed requests yet.")
        else:
            rows = [
                {
                    "Name": r["fullname"], "Email": r["email"], "Status": r["status"].title(),
                    "Reviewed by": r["reviewed_by"] or "—", "Reviewed at": r["reviewed_at"] or "—",
                }
                for r in reviewed
            ]
            st.dataframe(rows, hide_index=True, width="stretch")
...
def admin_page() -> None:
    ...
    notifications_label = f"Notifications ({pending_count})" if pending_count else "Notifications"

    tab_notifications, tab_overview, tab_create, tab_manage, tab_uploads, tab_activity = st.tabs(
        [notifications_label, "System overview", "Create user", "Manage users", "Data uploads", "Activity log"]
    )
